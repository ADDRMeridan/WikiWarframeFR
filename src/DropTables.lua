...
local function tableSort(theTable, sortCol, ascend)
    local new local function sorter(r1, r2)
                    if(ascend) then
...
local function formatDropString(drop, frame)
    ...
    if(dropType == "MorceauArme") then
      result = result..Icon._Item(drop.IName[1], "text", nil)..' - '..Tooltip._tooltipText(drop.IName[2], "Weapon")
    ...
    elseif (dropType == "Relique") then
        if(Void.getRelic(Shared.splitString(drop.IName, " ")[1], Shared.splitString(drop.IName, " ")[2]).isVaulted ~= 1) then
...
function p.getSingleRotationRewards(frame)
    ...
    if(theMission == nil) then
        return "ERROR: No mission found for the following parameters: ("..MissionType..","..MissionCat..")"
    end
...
function p.getRewardTable(frame)
    ...
    local theMission = p.getMission(MissionType, MissionCat)
    if(theMission == nil) then
        return "[[".."Catégorie:Pages_avec_des_erreurs_de_script]]ERROR: No mission found with '"..MissionType.."' and '"..MissionCat.."'."
    end
...
function p.getItemDropList(frame)
        ...
        for i, item in pairs(finalTable) do
            ...
            for j, alias in pairs(item) do
                ...
                for k, thisDrop in pairs(checked[alias]) do
                    ttip = ttip..'Rotation '..thisDrop.Rotation..': '..asPercent(thisDrop.Chance, 2)
                    end
                
                if j > 1 then result = result..', ' end
                result = result..'<span style="border-bottom: 1px dotted;" title="'..ttip..'">'..shortName..'</span>'
    ...
    if(Shared.tableCount(Drops) > 0) then
        ...
        for i, Drop in pairs(Drops) do
            result = result.."<br/>"..linkSyndicate(Drop.SName)..' '..Icon._Syndicat("Drapeau","","x20")..' '..Shared.formatnum(Drop.Cost)
        end
    end
    ...
    if(Shared.tableCount(Drops) > 0) then
        ...
        for i, Drop in pairs(Drops) do
            result = result.."<br/>"..linkEnemy(Drop.EName)..space..asPercent(Drop.Chance)
        end
    end
