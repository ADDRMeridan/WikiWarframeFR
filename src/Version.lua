...
function p.getVersion(name)
    ...
    if(Shared.containsCharChiantFR(name)) then
        ...
        for i, Version in pairs(VersionData["Versions"]) do
            ...
            if(string.upper(newVName) == string.upper(newName) or string.upper(newVSName) == string.upper(newName)) then
                return Version, i
            end
        ...
        for i, Version in pairs(VersionData["Versions"]) do
            if(string.upper(Version.Name) == string.upper(name) or string.upper(Version.ShortName) == string.upper(name) or (Version.Aliases ~= nil and Shared.contains(Version.Aliases, name, true))) then
                return Version, i
            end
        end
    end
...
function p._getVersionLink(name, color, short)
    ...
    if(Version == nil) then
        return "[[".."Category:Template Error]]<span style=\"color:red;\">ERROR: No such version found '"..name.."'</span>"
    end
    ...
    if color ~= nil then
        return "[["..Version.Link.."|<span style=\"color:"..color..";\">"..name.."</span>]]"
    else
...
local function UpdateTrim(str)
  return (str:gsub("^%a*%s(.-)$", "%1"))
end
