...
function p.getWarframes(validateFunction)
    ...
    for i, wframe in Shared.skpairs(WarframeData["Warframes"]) do
        if(i ~= "Equinox Forme Diurne" and i ~= "Equinox Forme Nocturne") then
...
function p.getWarframeComparisonTable(frame)
    return getWarframeComparisonTable(p.getWarframes(function(wf) return true end))
end
 
function p.getWarframeRank30ComparisonTable(frame)
    return getWarframeComparisonTable(p.getWarframes(function(wf) return true end), true)
end
 
function p.getConclaveWarframeComparisonTable(frame)
    return getWarframeComparisonTable(p.getConclaveWarframes(function(wf) return true end))
end
 
function p.getConclaveWarframeRank30ComparisonTable(frame)
    return getWarframeComparisonTable(p.getConclaveWarframes(function(wf) return true end), true)
end
...
function p.getReleaseDateRow(warframe,prime)
    ...
    result = result..'\n| '..Tooltip._tooltipText(warframe.Name, 'Warframe')..' || '..p._getValue(warframe, 'SEX')..' || '..outputReleaseDate..' || '
...
function p.getReleaseDateTable(frame)
    ...
    elseif(category == "FEMELLE") then
        warframes = p.getWarframes(function(wf) return wf.Sex == "Femelle" end)
    elseif(category == "MALE") then
        warframes = p.getWarframes(function(wf) return wf.Sex == "Mâle" end)
    end
...
function p._tooltip(wfName, conclave)
    ...
    result = result..halfTable..infoHeader("Santé")..space..Value("HealthPlusMinimal",true,true)..halfTable..infoHeader("Armure")..space..Value("ArmorPlusMinimal",true,true)
    result = result..newRow..halfTable..infoHeader("Bouclier")..space..Value("ShieldPlusMinimal",true,true)..halfTable..infoHeader("Energie")..space..Value("EnergyPlusMinimal",true,true)
    result = result..newRow..'\n| style=\"text-align:center;\" colspan=2 |'..infoHeader("Polarités")..space..whitePols("AuraPolarity")..' | '..whitePols("ExilusPolarity")..' | '..whitePols("Polarities")..tableEnd
...
function p.getWarframeArmorList(frame)
    ...
    local keys = {}
    for k in pairs(armorSet) do table.insert(keys, k) end
