...
local function getType(dName)
    if (DamageData["Types"][dName]) then return DamageData["Types"][dName] end
    return nil
end
...
function p.tooltip(frame)
    ...
    local supStart =
        '<div style=\"position:relative; display:inline-block; width:7px;\"><div style=\"position:absolute; font-size:10px;bottom: 0px;left: 0px;\">' -- for note number superscript
    local supEnd = '</div></div>' -- ending the previous one

    -- table locals
    local newRow = '\n|-'
    local spacer = '\n| class=\"Spacer\" style=\"' .. bgText .. colorBorder ..
                       '; ' .. bgText .. colorBorder .. colorBorderT .. '\"|'
    local dataRowStart = '\n| class=\"Data\" style=\"' .. bgText ..
                             colorBackground .. '; ' .. bgText ..
    ...
    local noteRow = dataRowStart ..
                        ' font-size:10px; line-height:12px; padding:0; white-space:inherit;\"|'

    local result = '{| class=\"Damage Sub\" style=\"border:3px solid ' ..
                       colorBorder .. '; border:3px solid ' .. colorBorder ..
                       colorBorderT .. ';\"' .. newRow

    if dType.Types then -- if a combined element, add icons for the two elements
        cTypes = Icon._Proc(dType.Types[1], 'text') .. ' + ' ..
                     Icon._Proc(dType.Types[2], 'text')
        result = result .. typeRow .. Icon._Proc(dName, 'text') .. newRow ..
                     typeRow .. cTypes .. newRow .. spacer .. newRow
    else
        result = result .. typeRow .. Icon._Proc(dName, 'text') .. newRow ..
                     spacer .. newRow -- only adding an icon for the damage type itself if not a combined element
    end
    if dType.Positives then -- if deals increased damage to health types, show those
        ...
        for key, pValue in pairs(dType.Positives) do
            pCount = pCount + 1
            pText = pText .. pValue[1] .. pSpan .. ' +' .. pValue[2] .. '%' ..
                        endSpan
            if pLenght > pCount then pText = pText .. '<br>' end
        end
    ...
    if dType.Negatives then -- if deals reduced damage to health types, show those
        ...
        for key, nValue in pairs(dType.Negatives) do
            nCount = nCount + 1
            nText = nText .. nValue[1] .. nSpan .. ' -' .. nValue[2] .. '%' ..
                        endSpan
            if nLenght > nCount then nText = nText .. '<br>' end
        end
    ...
    if dType.Bypass then -- if bypasses some protections, show them
        if dType.BypassNotes then
            for k, bNote in pairs(dType.BypassNotes) do
                ...
                bNotes = bNotes .. supStart .. k .. supEnd ..
                             (DamageData["Notes"][bNote])
            end
        end
        local bLenght = Shared.tableCount(dType.Bypass)
        for j, bName in pairs(dType.Bypass) do
            bTypes = bTypes .. bName .. bNAText .. supStart .. bNoteNumber ..
                         supEnd
            if bLenght > j then bTypes = bTypes .. '<br>' end
        end
    ...
    if dType.Status then -- show status effects
        ...
        for f, sName in pairs(dType.Status) do
            sText = sText .. sName
            if sLenght > f then sText = sText .. '<br>' end
        end
        if dType.StatusNotes then
            for i, sNote in pairs(dType.StatusNotes) do
                sNoteNumber = sNoteNumber .. i .. ' '
                sNoteText = sNoteText .. supStart .. i .. supEnd ..
                                (DamageData["Notes"][sNote]) .. ' '
            end
...
function p.getTypeStatus(frame)
    ...
    if (type.Status == nil) then return nil end
...
function p.getHealthTypeVsDamageType(frame)

    local healthName = frame.args ~= nil and frame.args[1] or nil
    local gameMode = frame.args ~= nil and frame.args[2] or nil

    return p._getHealthTypeVsDamageType(healthName, gameMode)
end

function p._getHealthTypeVsDamageType(healthName, gameMode)
